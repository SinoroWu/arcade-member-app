...
export class RewardService {
  ...
  async getRewardItems() {
    if (dbManager.prisma && !dbManager.isUsingMock) {
      return dbManager.prisma.rewardItem.findMany({
        where: { isActive: true },
        orderBy: { requiredTickets: 'asc' },
      });
    }
    return Array.from(dbManager.memoryStore.rewardItems.values()).filter((i) => i.isActive);
  }

  // 2. 會員使用彩票或紅利兌換獎品/票券
  async redeemRewardItem(userId: string, rewardItemId: string) {
    let item: any = null;
    if (dbManager.prisma && !dbManager.isUsingMock) {
      item = await dbManager.prisma.rewardItem.findUnique({ where: { id: rewardItemId } });
    } else {
      item = dbManager.memoryStore.rewardItems.get(rewardItemId);
    }
    ...
    if (dbManager.prisma && !dbManager.isUsingMock) {
      await dbManager.prisma.userVoucher.create({
        data: {
          id: voucher.id,
      ...
      dbManager.memoryStore.userVouchers.set(voucher.id, voucher);
    }
  ...
  async getUserVouchers(userId: string) {
    if (dbManager.prisma && !dbManager.isUsingMock) {
      return dbManager.prisma.userVoucher.findMany({
        where: { userId },
    ...
    const list = Array.from(dbManager.memoryStore.userVouchers.values())
      .filter((v) => v.userId === userId)
      .map((v) => {
        const rewardItem = dbManager.memoryStore.rewardItems.get(v.rewardItemId);
        return { ...v, rewardItem };
      });
  ...
  async redeemVoucherByStaff(voucherCode: string, staffId = 'STAFF_01') {
    let voucher: any = null;

    if (dbManager.prisma && !dbManager.isUsingMock) {
      voucher = await dbManager.prisma.userVoucher.findUnique({
        where: { voucherCode },
        include: { rewardItem: true, user: true },
      });
    } else {
      for (const v of dbManager.memoryStore.userVouchers.values()) {
        if (v.voucherCode === voucherCode) {
          voucher = v;
          voucher.rewardItem = dbManager.memoryStore.rewardItems.get(v.rewardItemId);
          voucher.user = dbManager.memoryStore.users.get(v.userId);
          break;
        }
      }
    }
    ...
    if (dbManager.prisma && !dbManager.isUsingMock) {
      await dbManager.prisma.userVoucher.update({
        where: { id: voucher.id },
