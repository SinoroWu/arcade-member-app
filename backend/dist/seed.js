...
async function seedInitialData() {
    ...
    machines.forEach((m) => {
        db_1.dbManager.memoryStore.machines.set(m.id, m);
    });
    ...
    rewards.forEach((r) => {
        db_1.dbManager.memoryStore.rewardItems.set(r.id, r);
    });
    // 3. 建立測試示範會員 (Demo Member)
    try {
        const demoUser = await auth_service_1.authService.register({
            account: '0912345678',
        ...
        const cardUser = await auth_service_1.authService.register({
            account: '88888888',
            password: '8888',
